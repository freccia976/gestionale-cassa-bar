document.addEventListener("DOMContentLoaded", () => {
  ...
  function settimanaCorrente() {
    return settimanaDaData(new Date());
  }
  ...
  btnDettaglio.onclick = () => {
    ...
    caricaDettaglio(settimanaCorrente());
  };
...
function apriPopupMese(anno, mese, movimentiMese) {
  ...
  calcolaSettimane(movimentiMese).forEach(s => {
    ...
    btn.onclick = () => {
      settimanaSelezionata = s;

      popupMese.classList.add("hidden");        // ✅ CHIUDE POPUP MESE
      popupSettimana.classList.remove("hidden");// ✅ APRE POPUP SETTIMANA
      caricaDettaglio(s);
    };
  ...
  function generaPDF(titolo, movs) {
    ...
    movs.forEach(m => {
      let riga = `${formattaData(m.data)} - €${m.importo.toFixed(2)}`;
      if (m.tipo === "entrata") {
        riga += m.metodo === "contanti" ? " (Contanti)" : " (POS)";
      } else {
        riga += ` - ${m.fornitore}`;
      }
      doc.text(riga, 14, y);
  ...
  btnExportMese.onclick = () => {
    if (!meseCorrenteMovimenti.length) return;
    generaPDF(titoloMese.textContent, meseCorrenteMovimenti);
  };

  btnExportSettimana.onclick = () => {
    if (!settimanaSelezionata) {
      alert("Seleziona prima una settimana");
      return;
    }
    const movs = meseCorrenteMovimenti.filter(m => {
      const d = new Date(m.data);
      return d >= settimanaSelezionata.lunedi && d <= settimanaSelezionata.sabato;
    });
    generaPDF("Settimana selezionata", movs);
  };
